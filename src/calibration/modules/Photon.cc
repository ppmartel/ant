...
Photon::Photon(const detector_ptr_t& det,
    ...
    PhotonPeak(det,"PhotonPeak",defaultPhotonPeak)
{
...
void Photon::ApplyTo(const readhits_t& hits)
{
    ...
    for(TDetectorReadHit& dethit : dethits) {
        ...
        if(!dethit.RawData.empty()) {
            ...
            for(const double& conv : Converter->Convert(dethit.RawData)) {
                ...
                value.Calibrated -=PhotonPeak.Get(dethit.Channel);
...
std::list<Updateable_traits::Loader_t> Photon::GetLoaders()
{
...
void Photon::UpdatedTIDFlags(const TID& id)
{
